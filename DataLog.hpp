...
int DataLog<LogEntry>::readEntry(int number, LogEntry* entry){
  ...
  if(number < 0){
    ...
    for(int i=0; i<number; i++){
      address += sizeof(LogEntry)+sizeof(uint16_t);
      if(address > LOG_END_ADDRESS - sizeof(LogEntry)+sizeof(uint16_t)) address = LOG_START_ADDRESS;
    }
  }
...
int DataLog<LogEntry>::writeEntryToEEPROM(int address, uint16_t index, LogEntry* entry){
  if(address > LOG_END_ADDRESS-(sizeof(LogEntry)+sizeof(uint16_t))) return -1;
  EEPROM.begin(EEPROM_MAX_SIZE);
  EEPROM.put(address, index);
...
int DataLog<LogEntry>::readEntryFromEEPROM(int address, LogEntry* entry){
  ...
  EEPROM.begin(EEPROM_MAX_SIZE);
  uint16_t index;
  EEPROM.get(address, index);
  EEPROM.get(address+sizeof(index), *entry);
  EEPROM.end();
...
void DataLog<LogEntry>::getAddressToOverwrite(int &address, uint16_t &index){
  ...
  while((index == lastindex+1 || index == lastindex) && index != 0xFFFF && index != 0x0){
    ...
    if(address > LOG_END_ADDRESS - (sizeof(LogEntry)+sizeof(index))) address = LOG_START_ADDRESS;
    EEPROM.get(address, index);
  }
  EEPROM.end();
